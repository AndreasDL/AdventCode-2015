...
//found at github.com/Workiva/go-datastructures/
//MODIFIED to use String method, so we don't have problems with unhashable types
...
func (pq *PriorityQueue) Put(items ...Item) error {
	...
	for _, item := range items {
		if pq.allowDuplicates {
			pq.items.push(item)
		} else if _, ok := pq.itemMap[item.String()]; !ok {
			pq.itemMap[item.String()] = struct{}{}
			pq.items.push(item)
		}
	}
...
func NewPriorityQueue(hint int, allowDuplicates bool) *PriorityQueue {
	return &PriorityQueue{
		...
		itemMap:         make(map[string]struct{}, hint),
